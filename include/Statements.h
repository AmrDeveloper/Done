...
public:
  ...
  FunctionStatement(Token n, Token t, std::vector<Parameter> params,
                    std::vector<Statement *> body)
...
public:
  ...
  std::vector<Parameter> parameters;
  Statement *body;
  LineFunctionStatement(Token n, Token t, std::vector<Parameter> params,
                        Statement *body)
      : name(std::move(n)), returnType(std::move(t)),
...
public:
  Expression *condition;
  std::vector<Statement *> body;
  WhileStatement(Expression *c, std::vector<Statement *> b)
      : condition(c), body(std::move(b)) {}
  void accept(class StatementVisitor *visitor) { visitor->visit(this); }
};
...
public:
  Expression *condition;
  std::vector<Statement *> body;
  DoWhileStatement(Expression *c, std::vector<Statement *> b)
      : condition(c), body(std::move(b)) {}
  void accept(class StatementVisitor *visitor) { visitor->visit(this); }
};
...
public:
  ...
  ArrayStatement(Token n, Token t, Expression *size, MemoryType type,
                 bool isInit, Expression *val)
      : name(std::move(n)), type(std::move(t)), size(size), memoryType(type),
        isInitialized(isInit), value(val) {}
