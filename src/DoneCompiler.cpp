...
void DoneCompiler::compile() {
  ...
  std::string cSourcefilePath = compilerOptions->mainSourceFilePath + "/" +
                                compilerOptions->generatedFileName;
  writeFileContent(cSourcefilePath, cSourceCode);

  if (compilerOptions->generateExecutable)
    generateExecutable();
  if (compilerOptions->runExecutable)
    runExecutable();
}
...
void DoneCompiler::generateExecutable() {
  ...
  stream << " ";
  stream << compilerOptions->mainSourceFilePath << "/"
         << compilerOptions->generatedFileName;
  stream << " ";
